...
class JsonInputReader():
    ...
    def _parse_sentiments(self, jsentiments, entities, dataset,sen_id) -> List[Sentiment]:
        ...
        for jsentiment in jsentiments:
            # For VA regression: parse VA scores from string "V.VV#A.AA"
            va_string = jsentiment['type']
            if '#' in va_string:
                # Parse VA scores
                valence, arousal = map(float, va_string.split('#'))
                # Create a dummy sentiment type with VA scores
                ...
                sentiment_type = self._sentiment_types.get(va_string, self._sentiment_types['None'])
                sentiment_type.va_scores = None
            ...
            if hasattr(sentiment_type, 'symmetric') and sentiment_type.symmetric and reverse:
