...
class D2E2SModel(PreTrainedModel):
    ...
    def __init__(
        ...
        # Modified for VA regression: output 2 values (valence, arousal) instead of sentiment classes
    ...
    def _forward_train(
        ...
        senti_clf = torch.zeros(
            [batch_size, sentiments.shape[1], 2]  # VA regression: 2 outputs
    ...
    def _forward_eval(
        ...
        for i in range(0, sentiments.shape[1], self._max_pairs):
            ...
            # For VA regression: no sigmoid, direct output
            senti_clf[:, i : i + self._max_pairs, :] = chunk_senti_logits
